...
`markdown::blocks` 使用 `pulldown-cmark` 的源码偏移范围生成顶层 `MarkdownBlock`。默认即时模式
在同一个扁平编辑器工作区内只把活动块交给原生 `TextEdit`，其余块交给原生 CommonMark 渲染器。
点击排版块会原地进入编辑，点击编辑区空白或按 Escape 会立即恢复排版。活动块更新后通过原字节
范围写回同一个 `Document`。编辑事务立即更新正文、dirty 状态和紧凑补丁；大纲、统计与块
索引在 120 ms 输入空闲窗口后统一刷新，混合视图请求块时则立即刷新，避免每个按键同步解析
整篇文档。
