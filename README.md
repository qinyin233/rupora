...
- 默认启用单画布即时编辑：点击排版段落原地编辑，离开后立即恢复最终排版
- VS Code 风格的原生浅色/深色工作台、编辑器标签、资源管理器和紧凑状态栏
- 源码、只读预览和兼容用经典分屏模式仍可随时切换
...
Windows 也可以先构建再直接启动调试程序；调试版和发布版都使用纯 GUI 子系统，不会额外弹出
命令行窗口：

```powershell
cargo build --locked
.\target\debug\rupora.exe
```
...
P1–P8 已逐项完成。默认即时编辑采用稳定块 ID：非活动块显示最终排版，活动块在同一位置
显示与内容层级匹配的编辑控件。它仍不是 Typora 的逐字形富文本模型；跨块富文本选择、隐藏
活动块内全部 Markdown 标记和多光标仍属于后续编辑器研究。
